-- Path scripthooked:lua\\effects\\eff_bulletdrop.lua"
-- Scripthooked by ???
...
function EFFECT:Init(data)
    ...
    if !hg.ammotypes[ammotype].NoSpin and (math.random(8) < (data:GetMagnitude() or 1)) then self.AddVelocity = math.random(4) * (data:GetMagnitude() or 1) * (math.random(2) == 1 and -1 or 1) end
...
function EFFECT:Think()
    ...
    self.Velocity = self.Velocity - vector_up * 0.05
